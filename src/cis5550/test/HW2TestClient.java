  ...
  void runTests(Set<String> tests) throws Exception {
    ...
    if (tests.contains("post"))
      try {
        ...
        if (!r.body().equals("req.body() returned: " + theBody))
          testFailed("The server returned a response, but it was not the string we expected. We sent a request with '"
    ...
    if (tests.contains("qparam"))
      try {
        ...
        Socket s = openSocket(8080);
        PrintWriter out = new PrintWriter(s.getOutputStream());
        String val1 = randomAlphaNum(5, 5), val2 = randomAlphaNum(5, 5), val3 = randomAlphaNum(5, 5),
        ...
        if (!r.body().equals(expectedResponse))
          testFailed("The server returned a response, but it was not the string '" + expectedResponse
